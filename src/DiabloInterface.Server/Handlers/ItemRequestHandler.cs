...
namespace Zutatensuppe.DiabloInterface.Server.Handlers
{
    public class ItemInfo
    {
        ...
        //
        // the string is stored in the BaseItem property
        // for the outside world, but Class should be used instead
        // in the future, as that is same across different localizations
        ...
        private ItemInfo(D2Unit item, D2Unit player, UnitReader unitReader, IInventoryReader inventoryReader)
        {
            Class = item.eClass;
            ItemName = unitReader.GetFullItemName(item);
            Properties = unitReader.GetMagicalStrings(item, player, inventoryReader);
            ...
            //              var ix = new D2Unit();
            //              ix.eClass = i;
            //              ix.eType = D2UnitType.Item;
            //              n = unitReader.GetItemName(ix);
            //              Logger.Debug($"{{{i}, \"{n}\"}},");
            //              i++;
            //          } while (n != null);

            var itemQuality = unitReader.GetItemQuality(item);
            BaseItem = BaseItemName(item, itemQuality);
            Logger.Debug(BaseItem);
            if (BaseItem == null)
            {
                BaseItem = unitReader.GetFullItemName(item);
            }
            Quality = QualityColor(itemQuality);
        }

        private string BaseItemName(D2Unit item, ItemQuality quality)
        {
            ...
            switch (quality)
            {
                ...
                case ItemQuality.Unique:
                    return null;
                default:
                    return item.eClass < BaseNamesNormal.Length
                        ? BaseNamesNormal[item.eClass]
                        : null;
            }
        }
